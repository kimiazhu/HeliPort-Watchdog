...
1. 可选：启动后先等待「启动延迟探测」秒数（默认 0 = 不延迟）再开始，用于开机自启时等 HeliPort 就绪；
2. 按探测间隔（默认 1 秒）对远端 IP 执行一次 ICMP ping；
3. 连续失败时长达到阈值（默认 10 秒）判定网络不通；
4. 通过 AppleScript（System Events 辅助功能自动化）点击 HeliPort 菜单栏菜单中的 Wi-Fi 电源开关（NSSwitch）：关闭 1 秒后再打开；
5. 修复后进入宽限期（不少于 30 秒），等待 Wi-Fi 重连，期间不计失败，避免在重连过程中反复触发重启；
6. 关→开为幂等操作，且重开失败会自动重试，不会把 Wi-Fi 留在关闭状态。
...
| 启动延迟探测（秒，0 不延迟） | 启动后先等待多少秒再开始探测，用于开机自启时等 HeliPort 就绪，避免启动初期持续失败误触发重启 | `0` | 0–86400 整数 |

配置的保存与生效：

- 点击居中的**保存配置**按钮：先校验，通过后**立即生效**（运行中的引擎下一探测周期即用新值，无需重启应用），同时持久化并在日志区回显生效值；例外：**「启动延迟探测」仅在下一次启动应用时生效**（延迟只在引擎启动时等待一次）；
- 配置存于 UserDefaults（key `watchdog.config.v1`），下次启动**自动加载**；记录缺失或损坏时回落上表默认值；旧版本存档缺少「启动延迟探测」字段时该回落默认值、其余字段保留；
...
- **日志不落盘**：仅保存在窗口内存中（上述环形缓冲即唯一存储），应用退出即清空，也不写入任何文件；CLI 版日志同样只输出到 stdout，需要留存时自行重定向（如 `./heliport-watchdog.sh | tee watchdog.log`）。
...
- **开机自启建议配合「启动延迟探测」**：开机后 HeliPort 就绪并连上 Wi-Fi 需要一段时间，可在配置区把「启动延迟探测」设为若干秒（如 60），避免启动初期对未就绪的 HeliPort 误触发重启（该延迟在每次启动应用时生效一次）。
...
swift test             # 引擎状态机等单元测试（当前 16 个用例）
```
...
| `-delay <秒>` | 启动后延迟多少秒再开始探测（0 = 不延迟；开机自启时可先等 HeliPort 就绪） | `0` |
